...
function hasAnyFieldValue(row: Row, fields: readonly ExtraFieldConfig[]): boolean {
  return fields.some((field) => {
    if (
      !shouldShowTechnologyResultFields(row.step) &&
...
export function RowDetailsView({
  ...
  const showResultFields = shouldShowTechnologyResultFields(row.step);
  ...
  const showTechnologyFields =
    ...
    hasAnyFieldValue(row, visibleTechnologyFields);
      ...
      >
        ...
        {showTechnologyFields && (
          <>
            ...
            {visibleTechnologyFields.map((field) => {
