...
CREATE TABLE `fts_functions` (
    ...
    UNIQUE INDEX `fts_functions_fts_function_name_id_key`(`fts_function_name_id`),
...
CREATE TABLE `fts_function_details` (
    ...
    `feedback_source_id` INTEGER NULL,
    ...
    INDEX `fts_function_details_feedback_source_id_idx`(`feedback_source_id`),
...
-- AddForeignKey
ALTER TABLE `fts_function_details` ADD CONSTRAINT `fts_function_details_feedback_source_id_fkey` FOREIGN KEY (`feedback_source_id`) REFERENCES `type`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
